...
impl TermTui {
    ...
    /// Spawn a terminal with a command using default size (24x80)
    pub fn spawn_with_command(
        title: impl Into<String>,
        command: &str,
        args: &[&str],
    ) -> Result<Self> {
        Self::spawn_with_command_and_size(title, command, args, 24, 80)
    }

    /// Spawn a terminal with a command and specific size
    pub fn spawn_with_command_and_size(
        title: impl Into<String>,
        command: &str,
        args: &[&str],
        rows: u16,

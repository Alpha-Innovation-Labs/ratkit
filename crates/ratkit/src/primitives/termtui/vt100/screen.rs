...
pub struct Screen {
    ...
    grid: crate::primitives::termtui::vt100::grid::Grid,
    alternate_grid: crate::primitives::termtui::vt100::grid::Grid,

    attrs: crate::primitives::termtui::vt100::attrs::Attrs,
    saved_attrs: crate::primitives::termtui::vt100::attrs::Attrs,
...
impl Screen {
    ...
    pub(crate) fn new(size: Size, scrollback_len: usize) -> Self {
        let grid = crate::primitives::termtui::vt100::grid::Grid::new(size, scrollback_len);
        Self {
            feed_buf: Vec::new(),

            grid,
            alternate_grid: crate::primitives::termtui::vt100::grid::Grid::new(size, 0),

            attrs: crate::primitives::termtui::vt100::attrs::Attrs::default(),
            saved_attrs: crate::primitives::termtui::vt100::attrs::Attrs::default(),
    ...
    fn grid(&self) -> &crate::primitives::termtui::vt100::grid::Grid {
    ...
    fn grid_mut(&mut self) -> &mut crate::primitives::termtui::vt100::grid::Grid {
        if self.mode(MODE_ALTERNATE_SCREEN) {
        ...
        if width == 0 {
            ...
            if width > 1 {
                ...
                let next_cell = self
                    ...
                    // into account.
                    .unwrap();
                next_cell.clear(crate::primitives::termtui::vt100::attrs::Attrs::default());
                self.grid_mut().col_inc(1);
            }
        }
    }
            ...
            ("", _, "", b'm') => {
                ...
                while idx < params.len() {
                    ...
                    match param {
                        ...
                        38 => {
                            let (color, consumed) = parse_sgr_color(&params[idx + 1..]);
...
fn parse_sgr_color(params: &[&str]) -> (Option<Color>, usize) {
    ...
    match mode {
        "2" => {
            ...
            let r = parse_sgr_u8(params[1]);
            let g = parse_sgr_u8(params[2]);
            let b = parse_sgr_u8(params[3]);
...
fn parse_sgr_u8(param: &str) -> Option<u8> {
    if param.is_empty() {
        return None;
    }
    let value: u16 = param.parse().ok()?;
